...
orden = ["tk_mas","tk_menos","tk_mult","tk_div","tk_mod","tk_asig","tk_mas","tk_menor","tk_mayor","tk_menor_igual","tk_mayor_igual","tk_igual","tk_y","tk_o","tk_div","tk_neg","tk_dosp","tk_pyc","tk_coma","tk_punto","tk_par_izq","tk_par_der"]
...
def SIG_OPERACION_MIXTA_ANIDADA():
	...
	if token == "tk_par_der" :
		...
		syntaxErr( 'tk_par_der', 'tk_mas', 'tk_menos', 'tk_div', 'tk_mult', 'tk_mod' )

def SIG_OPERACION_MIXTA():
	global token
	if token == "tk_par_der" or token == "tk_coma" or token == "tk_pyc" or token == "tk_y" or token == "tk_o" or token == "tk_igual" or token == "tk_mayor" or token == "tk_menor" or token == "tk_mayor_igual" or token == "tk_menor_igual" or token == "tk_dif" :
		pass
	elif token == "tk_mas" :
		match( "tk_mas" )
		OPERACION_MIXTA()
		...
		syntaxErr( 'tk_par_der', 'tk_coma', 'tk_pyc', 'tk_y', 'tk_o', 'tk_igual', 'tk_mayor', 'tk_menor', 'tk_mayor_igual', 'tk_menor_igual', 'tk_dif', 'tk_mas', 'tk_menos', 'tk_div', 'tk_mult', 'tk_mod' )
...
def ASIGNACION():
	...
	if token == "id" :
		...
		ASIGNACION()
		match( "tk_par_der" )
		SIGUIENTE_OPERACION_ASIGNACION()
	elif token == "tk_cadena" or token == "tk_caracter" :
		STRING()
	else:
...
def RET():
	global token
	if token == "fin_funcion" :
		pass
	else:
		syntaxErr( 'fin_funcion' )
...
def syntaxErr(*args):
    ...
    if arguments[1:-2] == "'funcion_principal'":
        ...
        exit()
    print('<' + str(Control.fila) + ',' + str(Control.columna) + '> Error sintactico: se encontro: "' + str(token) +'"; se esperaba:')
...
def main():
	...
	if token != 'EOF':
		syntaxErr('EOF')
	print('El analisis ha terminado exitosamente')
